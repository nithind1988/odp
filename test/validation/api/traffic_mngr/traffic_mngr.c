				     ...
				     uint32_t    num_pkts)
{
	...
	/* Disable the shaper, so as to get the pkts out quicker.
	 * We cannot do this if dynamic shaper update is not supported
	 * without it, set_shaper() can cause packet drops due to start/stop.
	 */
	if (dynamic_shaper_update)
		set_shaper(node_name, shaper_name, 0, 0);
				    ...
				    uint32_t    num_pkts)
{
	...
	/* Disable the shaper, so as to get the pkts out quicker.
	 * We cannot do this if dynamic shaper update is not supported as
	 * without it, set_shaper() can cause packet drops due to start/stop.
	 */
	if (dynamic_shaper_update)
		set_shaper(node_name, shaper_name, 0, 0);
