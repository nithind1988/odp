...
static int traffic_mngr_suite_common_init(void)
{
	...
	/* Mark all tests in this suite as inactive if tm cannot be supported */
	if (!supported)
		odp_cunit_set_inactive();

	/* Report suite init as success even if all tests are inactive */
	return 0;
}
...
{
	if (!tm_capabilities.tm_queue_query_mask)
		return ODP_TEST_INACTIVE;
