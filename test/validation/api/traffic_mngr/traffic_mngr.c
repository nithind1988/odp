...
static int test_overall_capabilities(void)
{
	...
	for (idx = 0; idx < num_records; idx++) {
		...
		CU_ASSERT((prio_modes[ODP_TM_PKT_PRIO_MODE_GLOBAL] != 0) ||
			  (prio_modes[ODP_TM_PKT_PRIO_MODE_OVERRIDE] != 0))
	}
...
get_supported_marking_wred_threshold(odp_tm_requirements_t *req)
{
	...
	for (i = 0; i < egress_capa_count; i++) {
		...
		if (egress_capa[i].tm_queue_threshold)
			req->tm_queue_threshold_needed = true;

		for (j = 0; j < egress_capa[i].max_levels; j++) {
			if (egress_capa[i].per_level[j].tm_node_threshold)
...
static int create_tm_system(void)
{
	...
	get_supported_marking_wred_threshold(&require);
