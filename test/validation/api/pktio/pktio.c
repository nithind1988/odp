...
static void pktio_test_pktout_compl(bool use_plain_queue)
{
	...
	for (i = 0; i < TX_BATCH_LEN; i++) {
		...
		if (use_plain_queue) {
			...
			qparam.sched.sync = ODP_SCHED_SYNC_ORDERED;
			qparam.sched.group = ODP_SCHED_GROUP_ALL;
			qparam.sched.lock_count = sched_capa.max_ordered_locks;
		}
	...
	for (i = 0; i < num_ifaces; ++i) {
		...
		if (i == 0) {
			CU_ASSERT_FATAL(pktio_capa.config.pktout.bit.tx_compl_ena);
...
static int pktio_check_pktout_compl_plain_queue(void)
{
	...
	if (ret < 0 || !capa.config.pktout.bit.tx_compl_ena ||
	    !capa.tx_compl.mode_all || !capa.tx_compl.queue_type_plain)
		return ODP_TEST_INACTIVE;
...
static int pktio_check_pktout_compl_sched_queue(void)
{
	...
	if (ret < 0 || !capa.config.pktout.bit.tx_compl_ena ||
	    !capa.tx_compl.mode_all || !capa.tx_compl.queue_type_sched)
		return ODP_TEST_INACTIVE;
